# Extacted from https://github.com/sandialabs/OperonSEQer/blob/main/OperonSEQer/extractCoverage.py
# minior changes so it can ingest the data
import sys
...
if __name__ == '__main__':
    ...
    new_rna = pd.DataFrame(rna)
    new_rna = new_rna[new_rna[2] == "gene"]
    rna = new_rna.values.tolist()
    ...
    for a in range(0,len(rna)-1):
        ...
        if chr_a != chr_b:
            ...
            if test != 'not valid' and testAI != 'not valid' and testBI != 'not valid' and testAB != 'not valid':
            	data.append([rna[a][8].split(';')[4].split('=')[1],rna[a+1][8].split(';')[4].split('=')[1],meanA,sdA,meanB,sdB,meanMid,sdMid,dist_genes,strand_a,strand_b,len(covA),len(covB),len(covMid),test[0],test[1],testAI[0],testAI[1],testBI[0],testBI[1],testAB[0],testAB[1]])
